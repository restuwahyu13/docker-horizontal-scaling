...
export class UsersService {
  ...
  async usersLogin(body: DTOUsersLogin): Promise<APIResponse> {
    try {
      ...
      const accessToken: string = jwtToken.accessToken.split('#')[0]
      const refreshToken: string = jwtToken.refreshToken
      const extractToken: string = jwtToken.accessToken.split('#')[1]

      await this.secrets.insert({ resource_by: checkUsername.id, resource_type: 'login', access_token: accessToken, refresh_token: refreshToken, extra_token: extractToken, expired_at: expiredAt(1, 'days') })
